...
import os
from .configs import Paths, DownloadOption


def set_yt_dlp_ffmpeg_path(program_dir: str):
    DownloadOption.Music.value["ffmpeg_location"] = os.path.join(
        program_dir, Paths.FFmpegFileDir.value
    )
    DownloadOption.Thumbnail.value["ffmpeg_location"] = os.path.join(
        program_dir, Paths.FFmpegFileDir.value
    )


def set_yt_dlp_path(program_dir: str, file_name: str):
    pass
